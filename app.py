...
if submitted_add and new_guids_text.strip():
    ...
    st.sidebar.success(f"Saved {len(new_guids)} GUID(s) to input file ({add_mode.lower()}).")
    st.rerun()
    ...
    if st.button("Run Agent", type="primary", disabled=current_df.empty):
        ...
        try:
            ...
            if all_runs:
                ...
                # SharePoint copy
                if SHAREPOINT_SYNC_DIR:
                    try:
                        os.makedirs(SHAREPOINT_SYNC_DIR, exist_ok=True)
                        if os.path.exists(html_path):
                            shutil.copy2(html_path, os.path.join(SHAREPOINT_SYNC_DIR, "comparison_report.html"))
                        if os.path.exists(HISTORICAL_FILE):
                            shutil.copy2(HISTORICAL_FILE, os.path.join(SHAREPOINT_SYNC_DIR, "GDC_RUN_LOG.xlsx"))
                        log(f"Files copied to SharePoint: {SHAREPOINT_SYNC_DIR}")
                    except Exception as e:
                        log(f"Warning: SharePoint copy failed: {e}")

                # Store path in session for report tab
